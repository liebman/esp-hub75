...
cfg_select! {
    ...
    hub75_use_parl_io => {
        ...
        pub(crate) type TxTransfer =
            esp_hal::parl_io::ParlIoTxTransfer<'static, LinearBcmBuf, Blocking>;
    }
    hub75_use_lcd_cam => {
...
impl<DM: esp_hal::DriverMode, FB: FrameBuffer + 'static> super::Hub75<DM, FB> {
    ...
    /// the next frame boundary (handled by the ISR). Returns a [`Hub75Swap`](crate::Hub75Swap)
    /// transfer object; call [`.wait_for_done()`](crate::Hub75Swap::wait_for_done)
    /// then [`.wait()`](crate::Hub75Swap::wait), or just `.wait()` directly for
    /// blocking.
    ///
    /// # Errors
    ///
