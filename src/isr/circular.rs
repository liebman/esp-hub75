...
/// kept alive in [`super::State`], so no `DMA::steal()` or stored closure is
...
pub(crate) trait FrameInterrupt {
    /// Clear a pending frame-boundary flag. Called by the ISR (to drain the
    /// handled boundary flag and any stale flag).
    fn clear_frame_interrupt(&self);
}

cfg_select! {
    hub75_use_i2s_parallel => {
        impl<B: DmaTxBuffer> FrameInterrupt for TxXfer<B> {
            fn clear_frame_interrupt(&self) {
                use esp_hal::i2s::parallel::I2sParallelInterrupt;
                // The boundary flag is `out_total_eof`: the armed chain ends
                // with `suc_eof` + `NULL` next, exactly like a linear
                // transfer, and the free-running (disarmed) ring never ends,
                // so the flag can only be latched at an armed pass boundary.
                self.clear_interrupts(I2sParallelInterrupt::TotalEof);
            }
        }
    }
    hub75_use_lcd_cam => {
