...
//! via ESP-IDF (std / FreeRTOS) on ESP32-S3.
//!
//! Unlike the bare-metal `hello_lcd_cam` example there is no `#![no_std]`,
//! no `#![no_main]`, no DMA descriptor array, and no DMA channel argument.
//! DMA is managed automatically by the ESP-IDF LCD driver.
//!
//! # Build
//!
//! ```sh
//! cargo build --example hello_lcd_cam_idf \
//!     --features esp32s3-idf \
...
//! # Pin mapping
//!
//! The GPIO numbers below match the bare-metal `hello_lcd_cam.rs` example.
...
// Allocate the frame buffer as a static in order to not overflow the stack.
