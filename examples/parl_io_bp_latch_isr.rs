...
//! the PARL_IO peripheral with bitplane framebuffer and a SmartLEDShield-style
//! latch circuit.
//!
//! Unlike `parl_io_bp_latch` which needs `Hub75::new_async()` plus an
...
//!
//! Because the ISR sends one plane at a time there is no 65535-byte PARL_IO
//! transfer limit, so 7 planes are used on both C5 and C6.
//!
